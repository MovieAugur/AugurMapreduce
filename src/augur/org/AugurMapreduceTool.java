...
public class AugurMapreduceTool extends Configured implements Tool {
	...
	public static class Reduce extends
		...
		public void reduce(Text movieName,
				Iterable<IndividualMetric> metricList, Context context) throws IOException, InterruptedException {
			MovieMetric movieMetric = new MovieMetric();
			int aCount = 0;
			int cCount = 0;
			int tCount = 0;
			int yCount = 0;
			final Iterator<IndividualMetric> metricIter = metricList.iterator();
			while(metricIter.hasNext()) {
				IndividualMetric metric = metricIter.next();
				...
				if(metric.type == CommentType.Numeric) {
					...
					movieMetric.metrics.put(type, metric.metricValue);
				}
				if(metric.type == CommentType.Text) {
					double tempMetricValue = 0;
					if(metric.source == CommentSource.RottenTomatoes) {
						if(metric.exType == ExtraCommentType.Audience) {
							tempMetricValue = (movieMetric.metrics.containsKey(MetricType.AudienceComment)) ? movieMetric.metrics.get(MetricType.AudienceComment).get() : 0.0;
							tempMetricValue += metric.metricValue.get();
							type = MetricType.AudienceComment;
							aCount++;
						}
						if(metric.exType == ExtraCommentType.Critic) {
							tempMetricValue = (movieMetric.metrics.containsKey(MetricType.CriticsComment)) ? movieMetric.metrics.get(MetricType.CriticsComment).get() : 0.0;
							tempMetricValue += metric.metricValue.get();
					...
					if(metric.source == CommentSource.Twitter) {
						tempMetricValue = (movieMetric.metrics.containsKey(MetricType.TwitterComment)) ? movieMetric.metrics.get(MetricType.TwitterComment).get() : 0.0;
						tempMetricValue += metric.metricValue.get();
						type = MetricType.TwitterComment;
						tCount++;
					}
					if(metric.source == CommentSource.YouTube) {
						tempMetricValue = (movieMetric.metrics.containsKey(MetricType.VideoComment)) ? movieMetric.metrics.get(MetricType.VideoComment).get() : 0.0;
						tempMetricValue += metric.metricValue.get();
						type = MetricType.VideoComment;
						yCount++;
					}
					movieMetric.metrics.put(type, new DoubleWritable(tempMetricValue));
				}
			}
			double Avg = movieMetric.metrics.get(MetricType.AudienceComment).get() / aCount;
			movieMetric.metrics.put(MetricType.AudienceComment, new DoubleWritable(Avg));
			Avg = movieMetric.metrics.get(MetricType.CriticsComment).get() / cCount;
			movieMetric.metrics.put(MetricType.CriticsComment, new DoubleWritable(Avg));
			Avg = movieMetric.metrics.get(MetricType.TwitterComment).get() / tCount;
			movieMetric.metrics.put(MetricType.TwitterComment, new DoubleWritable(Avg));
			Avg = movieMetric.metrics.get(MetricType.VideoComment).get() / yCount;
			movieMetric.metrics.put(MetricType.VideoComment, new DoubleWritable(Avg));
	...
	public int run(String[] args) throws Exception {
		...
		job.addArchiveToClassPath(new Path("s3://augurframework/include/nlpjars.tar.gz"));
		job.addFileToClassPath(new Path("s3://augurframework/include/nlp_file.properties"));
